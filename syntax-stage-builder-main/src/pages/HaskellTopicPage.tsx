...
import { haskellModules, type HaskellModule, type HaskellTopic } from "@/data/haskellCourse";

const HaskellTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const currentModule = useMemo<HaskellModule | null>(() => haskellModules.find((m) => m.id === Number(moduleId)) ?? haskellModules.find((m) => m.topics.some((t) => t.id === topicId)) ?? null, [moduleId, topicId]);
  const currentTopic = useMemo<HaskellTopic | null>(() => currentModule?.topics.find((t) => t.id === topicId) ?? null, [currentModule, topicId]);
  ...
  if (!currentModule || !currentTopic) return <div className="min-h-screen flex items-center justify-center"><Link to="/haskell-learning"><Button>Back to Haskell</Button></Link></div>;

  const topics = currentModule.topics;
  const idx = topics.findIndex((t) => t.id === currentTopic.id);
  const prev = idx > 0 ? topics[idx - 1] : null;
  const next = idx < topics.length - 1 ? topics[idx + 1] : null;
  const exercises = Array.from({ length: Math.max(currentTopic.exercises, 3) }, (_, i) => [`Explain "${currentTopic.title}".`, `Write Haskell code for "${currentTopic.title}".`, `Create questions about "${currentTopic.title}".`][i % 3]);

  return (
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b"><div className="container mx-auto px-6 py-4 flex justify-between"><div className="flex items-center gap-4">
        <BackButton label="Back to Module" fallback="/haskell-learning" className="mb-0" /><h1 className="text-xl font-bold">{currentModule.title}</h1></div><Badge>{currentTopic.difficulty}</Badge></div></header>
      ...
      <div className="container mx-auto px-6 pb-8 flex justify-between"><Button variant="outline" disabled={!prev} onClick={() => prev && navigate(`/haskell-learning/topic/${currentModule.id}/${prev.id}`)}>← Prev</Button><Button disabled={!next} onClick={() => next && navigate(`/haskell-learning/topic/${currentModule.id}/${next.id}`)}>Next →</Button></div>
    </div>
  );
};

export default HaskellTopicPage;
