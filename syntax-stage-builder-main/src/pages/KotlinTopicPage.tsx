...
import { kotlinModules, type KotlinModule, type KotlinTopic } from "@/data/kotlinCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Explain "${title}" to a beginner Android developer with a short Kotlin example.`,
    `Write a Kotlin snippet that demonstrates "${title}" in a realistic scenario.`,
    `Create three quiz questions about "${title}" and answer them.`,
    `Refactor a Java-style example into Kotlin using "${title}" idioms.`,
    `Check the Kotlin docs for "${title}" and summarize two best practices.`,
  ];
  const length = Math.max(count, MIN_EXERCISES);
  return Array.from({ length }, (_, index) => templates[index % templates.length]);
};

const KotlinTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const moduleNumber = moduleId ? Number(moduleId) : Number.NaN;

  const currentModule = useMemo<KotlinModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = kotlinModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return kotlinModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<KotlinTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Run this Kotlin example in Android Studio or the Kotlin REPL.)");
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Choose a topic from the Kotlin learning path to continue.</p>
          <Link to="/kotlin-learning">
            <Button>Back to Kotlin Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/kotlin-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with Kotlin code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/kotlin-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/kotlin-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
...
export default KotlinTopicPage;
