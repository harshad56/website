...
import { dartModules, type DartModule, type DartTopic } from "@/data/dartCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the key ideas of "${title}" and how they apply to Dart/Flutter development.`,
    `Write a short Dart snippet that demonstrates "${title}" in practice.`,
    `Create three interview questions about "${title}" and answer them yourself.`,
    `Refactor a simple example to use "${title}" and explain the benefits.`,
    `Check the official Dart documentation for "${title}" and note two best practices.`,
  ];
...
const DartTopicPage = () => {
  ...
  const currentModule = useMemo<DartModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = dartModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }
    if (topicId) {
      return dartModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }
    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<DartTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Use DartPad or a local environment to run this example.)");
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Choose a topic from the Dart learning path to continue.</p>
          <Link to="/dart-learning">
            <Button>Back to Dart Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <Link to={`/dart-learning?module=${currentModule.id}`}>
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Duration: {currentTopic.duration} • Difficulty: {currentTopic.difficulty}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="whitespace-pre-line leading-relaxed text-foreground">{currentTopic.content}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with Dart code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
                  className="w-full h-48 bg-muted text-foreground font-mono text-sm rounded-lg p-4 outline-none resize-none"
                  value={userCode}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/dart-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/dart-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
