...
import { rustModules, type RustModule, type RustTopic } from "@/data/rustCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Explain the core ideas of "${title}" with a concrete Rust code example.`,
    `Write a small Rust program that demonstrates "${title}" in a realistic scenario.`,
    `Create your own mini-cheatsheet of rules for "${title}" and test it with code.`,
    `Compare how "${title}" works in Rust vs in another language you know.`,
    `Review the Rust book or official docs for "${title}" and summarize two important tips.`
  ];
...
const RustTopicPage = () => {
  ...
  const currentModule = useMemo<RustModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = rustModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return rustModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<RustTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Compile with `cargo run` in your local setup.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the Rust learning path to continue.
          </p>
          <Link to="/rust-learning">
            <Button>Back to Rust Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/rust-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with Rust code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() =>
                      navigate(`/rust-learning/topic/${currentModule.id}/${topic.id}`)
                    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/rust-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/rust-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
