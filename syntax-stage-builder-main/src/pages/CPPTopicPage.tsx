...
import { cppModules, type CPPModule, type CPPTopic } from "@/data/cppCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the main ideas of "${title}" in C++ using your own words.`,
    `Write a short C++ program that applies "${title}" in a practical way.`,
    `Create three interview-style questions about "${title}" and answer them.`,
    `Document a bug you can solve using "${title}" and outline the fix in C++.`,
    `Review the official C++ reference for "${title}" and list two best practices.`,
  ];
...
const CPPTopicPage = () => {
  ...
  const currentModule = useMemo<CPPModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = cppModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return cppModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<CPPTopic | null>(() => {
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/cpp-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() =>
                      navigate(`/cpp-learning/topic/${currentModule.id}/${topic.id}`)
                    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/cpp-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/cpp-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
