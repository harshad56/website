...
import { csharpModules, type CSharpModule, type CSharpTopic } from "@/data/csharpCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the main ideas of "${title}" in your own words as if teaching a friend.`,
    `Build a small C# console app that applies "${title}" in a real-world scenario.`,
    `Write three interview-style questions about "${title}" and answer them yourself.`,
    `Review the official Microsoft docs for "${title}" and list two best practices you discovered.`,
    `Refactor an existing snippet using "${title}" to be cleaner and more idiomatic.`
  ];
  const length = Math.max(count, MIN_EXERCISES);
  return Array.from({ length }, (_, index) => templates[index % templates.length]);
};

const CSharpTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const moduleNumber = moduleId ? Number(moduleId) : Number.NaN;

  const currentModule = useMemo<CSharpModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = csharpModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return (
        csharpModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null
      );
    }
  ...
  const currentTopic = useMemo<CSharpTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Use your local C# environment to run this sample.)");
  };
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/csharp-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/csharp-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/csharp-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
...
export default CSharpTopicPage; 