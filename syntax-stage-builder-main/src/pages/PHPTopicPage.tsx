...
import { phpModules, type PhpModule, type PhpTopic } from "@/data/phpCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Explain the concept of "${title}" and show a small PHP example that uses it.`,
    `Modify an existing PHP snippet to apply "${title}" correctly.`,
    `Write three interview-style questions about "${title}" and answer them.`,
    `Sketch a simple web page feature where "${title}" is critical and outline the code.`,
    `Review the PHP manual entry for "${title}" and summarize two important details.`,
  ];
...
const PHPTopicPage = () => {
  ...
  const currentModule = useMemo<PhpModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = phpModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return phpModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<PhpTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Run this PHP code on a local server for real behavior.)");
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Choose a topic from the PHP learning path to continue.</p>
          <Link to="/php-learning">
            <Button>Back to PHP Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/php-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with PHP code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
            ...
            <Card className="sticky top-8">
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => navigate(`/php-learning/topic/${currentModule.id}/${topic.id}`)}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/php-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/php-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
