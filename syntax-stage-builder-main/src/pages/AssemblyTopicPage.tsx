...
import { assemblyModules, type AssemblyModule, type AssemblyTopic } from "@/data/assemblyCourse";

const AssemblyTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const currentModule = useMemo<AssemblyModule | null>(() => assemblyModules.find((m) => m.id === Number(moduleId)) ?? assemblyModules.find((m) => m.topics.some((t) => t.id === topicId)) ?? null, [moduleId, topicId]);
  const currentTopic = useMemo<AssemblyTopic | null>(() => currentModule?.topics.find((t) => t.id === topicId) ?? null, [currentModule, topicId]);
  ...
  if (!currentModule || !currentTopic) return <div className="min-h-screen flex items-center justify-center"><Link to="/assembly-learning"><Button>Back to Assembly</Button></Link></div>;

  const topics = currentModule.topics;
  const idx = topics.findIndex((t) => t.id === currentTopic.id);
  const prev = idx > 0 ? topics[idx - 1] : null;
  const next = idx < topics.length - 1 ? topics[idx + 1] : null;
  const exercises = Array.from({ length: Math.max(currentTopic.exercises, 3) }, (_, i) => [`Explain "${currentTopic.title}".`, `Write assembly for "${currentTopic.title}".`, `Debug an example of "${currentTopic.title}".`][i % 3]);

  return (
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b"><div className="container mx-auto px-6 py-4 flex justify-between"><div className="flex items-center gap-4">              <Link to={`/assembly-learning?module=${currentModule.id}`} replace>
        <Button variant="outline" size="sm">
          ← Back to Module
        </Button>
      </Link><h1 className="text-xl font-bold">{currentModule.title}</h1></div><Badge>{currentTopic.difficulty}</Badge></div></header>
      ...
      <div className="container mx-auto px-6 pb-8 flex justify-between"><Button variant="outline" disabled={!prev} onClick={() => prev && navigate(`/assembly-learning/topic/${currentModule.id}/${prev.id}`)}>← Prev</Button><Button disabled={!next} onClick={() => next && navigate(`/assembly-learning/topic/${currentModule.id}/${next.id}`)}>Next →</Button></div>
    </div>
  );
};

export default AssemblyTopicPage;
