...
const AITutor = () => {
  ...
  // Challenges for the Active Pulse widget
  const PULSE_CHALLENGES = [
    { title: "Linked List Implementation", description: "Implement a generic linked list with add, remove, and traverse methods." },
    { title: "Binary Search Tree", description: "Create a BST with efficient search, insertion, and deletion capabilities." },
    { title: "LRU Cache Design", description: "Design a Least Recently Used (LRU) cache with O(1) operations." },
    { title: "Valid Parentheses", description: "Check if a string has valid matching parentheses using a stack." },
    { title: "Merge K Sorted Lists", description: "Merge k sorted linked lists into one sorted linked list." }
  ];

  const [activeChallenge, setActiveChallenge] = useState(PULSE_CHALLENGES[0]);

  useEffect(() => {
    // Randomly select a challenge on mount
    const randomChallenge = PULSE_CHALLENGES[Math.floor(Math.random() * PULSE_CHALLENGES.length)];
  ...
  const handleUtilityTool = (toolLabel: string) => {
    ...
    let codeToAnalyze = analysisCode;

    // 2. Check if code is present, if not try to find in chat
    if (!codeToAnalyze.trim()) {
      const lastCodeMsg = [...messages].reverse().find(m => m.code);
      if (lastCodeMsg && lastCodeMsg.code) {
        setAnalysisCode(lastCodeMsg.code);
        codeToAnalyze = lastCodeMsg.code;
        toast({
          title: "Code Imported",
          description: "Imported the last code snippet from chat for analysis.",
        });
      } else {
        toast({
          title: "Code Required",
          description: "Please enter some code or ask the AI to generate some first.",
          variant: "warning"
        });
        return;
      }
    ...
    switch (toolLabel) {
      case "Structural Explanation":
        task = "Explain the architecture and data flow of this code in simple terms. Break it down component by component.";
        break;
      case "Fault Simulation":
        task = "Act as a QA engineer. Find potential bugs, edge cases, and security vulnerabilities in this code. Provide scenarios where it might fail.";
        break;
      case "Atomic Optimization":
        task = "Act as a Senior Engineer. Suggest ways to improve the performance (Big O) and readability of this code through refactoring. Provide the optimized code.";
        break;
    }

    // We need to pass the code explicitly since state update might not be immediate
    // But handleAnalyzeCode uses state, so let's rely on the setAnalysisCode above 
    // and wait a tick or just update the logic to accept code argument.
    // Since handleAnalyzeCode reads state, and we just set state, React batching might be an issue.
    // Let's modify handleAnalyzeCode to accept optional code or just wait.
    // Actually, best to pass it to the API call directly if possible, but handleAnalyzeCode is closed over scope.
    // A timeout is a simple hack, but accurate way is refactoring handleAnalyzeCode.
    // Let's assume the user will click "Start Deep Audit" if it doesn't auto run, 
    // OR we can manually call the API here.

    // For now, let's just populate it and show the tab. The user can click "Start Deep Audit".
    // Or even better, let's trigger it if we have code.

    setTimeout(() => {
      // Only trigger if we actually have code now
      if (codeToAnalyze) {
        handleAnalyzeCode(task, codeToAnalyze);
      }
    }, 100);
  };

  // Update handleAnalyzeCode to accept optional task and code
  const handleAnalyzeCode = async (customTask?: string, customCode?: string) => {
    const codeToProcess = customCode || analysisCode;
    ...
    try {
      const response = await apiService.aiTutorAnalyzeCode({
        code: codeToProcess,
        language: "javascript", // We should probably detect this or add a selector
        task: customTask
      });
      ...
      if (response.success) {
        ...
        toast({
          ...
          description: response.message || "Could not analyze code",
          variant: "destructive"
        });
      }
    } catch (error: any) {
      toast({
        title: error.title || "Analysis Error",
        description: error.message || "Failed to connect to analysis service",
        variant: error.variant || "destructive"
      });
    } finally {
      setIsAnalyzing(false);
            ...
            <div className="group relative rounded-[32px] overflow-hidden bg-indigo-600 shadow-2xl shadow-indigo-600/20">
              ...
              <div className="relative p-8 space-y-6">
                ...
                <div className="space-y-1">
                  <h5 className="text-white font-bold text-lg leading-tight">{activeChallenge.title}</h5>
                  <p className="text-indigo-100 font-medium text-sm opacity-90">{activeChallenge.description}</p>
                </div>
                <Button
                  variant="secondary"
                  className="w-full h-14 rounded-2xl bg-white text-indigo-600 hover:bg-indigo-50 font-black shadow-xl"
