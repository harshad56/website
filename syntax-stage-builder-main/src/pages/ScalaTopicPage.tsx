...
import { scalaModules, type ScalaModule, type ScalaTopic } from "@/data/scalaCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the key ideas of "${title}" and how they appear in idiomatic Scala code.`,
    `Write a short Scala program that demonstrates "${title}" in practice.`,
    `Create three interview-style questions about "${title}" and answer them yourself.`,
    `Refactor a Scala example to better use "${title}" and explain the improvements.`,
    `Check the official Scala docs for "${title}" and note two practical tips.`
  ];
  const length = Math.max(count, MIN_EXERCISES);
  return Array.from({ length }, (_, index) => templates[index % templates.length]);
};

const ScalaTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const moduleNumber = moduleId ? Number(moduleId) : Number.NaN;

  const currentModule = useMemo<ScalaModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = scalaModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return (
        scalaModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null
      );
    }
  ...
  const currentTopic = useMemo<ScalaTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Use scala/sbt to run this example locally.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the Scala learning path to continue.
          </p>
          <Link to="/scala-learning">
            <Button>Back to Scala Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/scala-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with Scala code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() =>
                      navigate(`/scala-learning/topic/${currentModule.id}/${topic.id}`)
                    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/scala-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/scala-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
