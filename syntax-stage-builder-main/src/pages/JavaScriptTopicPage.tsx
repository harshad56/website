...
import { javascriptModules, type JavaScriptModule, type JavaScriptTopic } from "@/data/javascriptCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the main ideas behind ${title} in your own words.`,
    `Create a mini demo that showcases ${title} in the browser or Node.js.`,
    `Write three interview-style questions about ${title} and answer them.`,
    `Document a bug you can solve using ${title} and outline the fix.`,
    `Research official docs for ${title} and note two best practices.`
  ];
  const length = Math.max(count, MIN_EXERCISES);
  return Array.from({ length }, (_, index) => templates[index % templates.length]);
};

const JavaScriptTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const moduleNumber = moduleId ? Number(moduleId) : Number.NaN;

  const currentModule = useMemo<JavaScriptModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = javascriptModules.find((module) => module.id === moduleNumber);
      if (match) {
        return match;
      }
    }
    ...
    if (topicId) {
      return (
        javascriptModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null
      );
    }
  ...
  const currentTopic = useMemo<JavaScriptTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Run the snippet in your browser devtools or Node REPL to see it live.");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the JavaScript learning path to continue exploring the curriculum.
          </p>
          <Link to="/javascript-learning">
            <Button>Back to JavaScript Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/javascript-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Reinforce this topic with guided prompts.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {exercises.map((exercise, index) => (
                  <div key={index} className="rounded-lg border p-4">
                    <h4 className="font-semibold mb-1">Exercise {index + 1}</h4>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use the sandbox to tweak the code sample.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
                ...
                <div className="flex gap-3 mt-4">
                  <Button onClick={() => setOutput("Run this snippet in your local environment to inspect output.")}>
                    ▶ Run Code
                  </Button>
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="sticky top-8">
              <CardHeader>
                ...
                <CardDescription>Jump to any lesson quickly.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => navigate(`/javascript-learning/topic/${currentModule.id}/${topic.id}`)}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/javascript-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/javascript-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
...
export default JavaScriptTopicPage;
