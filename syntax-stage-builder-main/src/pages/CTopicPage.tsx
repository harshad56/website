...
import { cModules, type CModule, type CTopic } from "@/data/cCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the key ideas of "${title}" and where they appear in real C programs.`,
    `Write a short C program that demonstrates "${title}" in practice.`,
    `Create three interview questions about "${title}" and answer them yourself.`,
    `Refactor a C code snippet to improve how it uses "${title}" and explain your changes.`,
    `Check a C reference for "${title}" and note two important safety considerations.`
  ];
...
const CTopicPage = () => {
  ...
  const currentModule = useMemo<CModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = cModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return cModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<CTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code compiled and executed successfully! (Use gcc/clang to run this example locally.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the C learning path to continue.
          </p>
          <Link to="/c-learning">
            <Button>Back to C Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/c-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with C code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
                ...
                <div className="flex gap-3 mt-4">
                  <Button onClick={() => setOutput("Code compiled successfully! (simulate output)")}>
                    ▶ Run Code
                  </Button>
            ...
            <Card className="sticky top-8">
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => navigate(`/c-learning/topic/${currentModule.id}/${topic.id}`)}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/c-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/c-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
