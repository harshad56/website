...
import { goModules, type GoModule, type GoTopic } from "@/data/goCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Explain the main ideas of "${title}" using a real-world analogy.`,
    `Write a small Go program that demonstrates "${title}" in practice.`,
    `Create three conceptual questions about "${title}" and answer them.`,
    `Refactor an existing snippet to use idiomatic Go patterns from "${title}".`,
    `Check the Go blog or official docs for "${title}" and summarize two best practices.`
  ];
...
const GoTopicPage = () => {
  ...
  const currentModule = useMemo<GoModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = goModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return goModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<GoTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Run it with `go run` in your local environment.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the Go learning path to continue.
          </p>
          <Link to="/go-learning">
            <Button>Back to Go Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/go-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with Go code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/go-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/go-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
