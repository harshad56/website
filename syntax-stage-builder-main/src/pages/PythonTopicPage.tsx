...
import { pythonModules, type PythonModule, type PythonTopic } from "@/data/pythonCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the core principles of ${title} in your own words.`,
    `Build a mini demo or notebook that applies ${title} in practice.`,
    `Create three interview-style questions about ${title} and answer them.`,
    `Document a real-world scenario where ${title} solves a problem.`,
    `Review the official docs for ${title} and list two best practices.`
  ];
  const length = Math.max(count, MIN_EXERCISES);
  return Array.from({ length }, (_, index) => templates[index % templates.length]);
};

const PythonTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const moduleNumber = moduleId ? Number(moduleId) : Number.NaN;

  const currentModule = useMemo<PythonModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = pythonModules.find((module) => module.id === moduleNumber);
      if (match) {
        return match;
      }
    }
    ...
    if (topicId) {
      return (
        pythonModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null
      );
    }
  ...
  const currentTopic = useMemo<PythonTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Use your local Python environment to run this sample.)");
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Choose a topic from the Python learning path to continue.</p>
          <Link to="/python-learning">
            <Button>Back to Python Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/python-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
                ...
                <div className="flex gap-3 mt-4">
                  <Button onClick={() => setOutput("Code executed successfully! (simulate output)")}>▶ Run Code</Button>
                  <Button variant="outline" onClick={() => setUserCode("")}>Reset</Button>
                </div>
                {output && (
            ...
            <Card className="sticky top-8">
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => navigate(`/python-learning/topic/${currentModule.id}/${topic.id}`)}
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/python-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/python-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
...
export default PythonTopicPage;
