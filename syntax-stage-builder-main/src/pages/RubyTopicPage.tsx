...
import { rubyModules, type RubyModule, type RubyTopic } from "@/data/rubyCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the key ideas of "${title}" and how they are used in real Ruby code.`,
    `Write a short Ruby script that demonstrates "${title}" in practice.`,
    `Create three interview-style questions about "${title}" and answer them yourself.`,
    `Refactor a small Ruby example to better use "${title}" and explain the improvements.`,
    `Check the official Ruby docs for "${title}" and note two best practices.`
  ];
...
const RubyTopicPage = () => {
  ...
  const currentModule = useMemo<RubyModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = rubyModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return rubyModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ?? null;
    }

    return null;
  }, [moduleNumber, topicId]);

  const currentTopic = useMemo<RubyTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Use a local Ruby interpreter to run this example.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the Ruby learning path to continue.
          </p>
          <Link to="/ruby-learning">
            <Button>Back to Ruby Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/ruby-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with Ruby code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
              ...
              <CardContent className="space-y-2">
                {topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() =>
                      navigate(`/ruby-learning/topic/${currentModule.id}/${topic.id}`)
                    }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/ruby-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/ruby-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
