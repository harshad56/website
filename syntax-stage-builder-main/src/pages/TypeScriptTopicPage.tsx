...
import { typescriptModules, type TypeScriptModule, type TypeScriptTopic } from "@/data/typescriptCourse";

const MIN_EXERCISES = 3;

const buildExercises = (title: string, count: number) => {
  const templates = [
    `Summarize the key ideas of "${title}" and how they improve JavaScript code.`,
    `Write a short TypeScript snippet that demonstrates "${title}" in practice.`,
    `Create three interview questions about "${title}" and answer them yourself.`,
    `Refactor a plain JavaScript example to use "${title}" and explain the benefits.`,
    `Check the official TypeScript docs for "${title}" and note two best practices.`,
  ];
  const length = Math.max(count, MIN_EXERCISES);
  return Array.from({ length }, (_, index) => templates[index % templates.length]);
};

const TypeScriptTopicPage = () => {
  const { moduleId, topicId } = useParams();
  const navigate = useNavigate();
  const moduleNumber = moduleId ? Number(moduleId) : Number.NaN;

  const currentModule = useMemo<TypeScriptModule | null>(() => {
    if (Number.isFinite(moduleNumber)) {
      const match = typescriptModules.find((module) => module.id === moduleNumber);
      if (match) return match;
    }

    if (topicId) {
      return (
        typescriptModules.find((module) => module.topics.some((topic) => topic.id === topicId)) ??
        null
      );
    }
  ...
  const currentTopic = useMemo<TypeScriptTopic | null>(() => {
  ...
  const runExample = () => {
    setOutput("Code executed successfully! (Use ts-node or a bundler to run this example locally.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            Choose a topic from the TypeScript learning path to continue.
          </p>
          <Link to="/typescript-learning">
            <Button>Back to TypeScript Course</Button>
          </Link>
        </div>
      </div>
    );
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <BackButton label="Back to Module" fallback="/typescript-learning" className="mb-0" />
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to experiment with TypeScript code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/typescript-learning/topic/${currentModule.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/typescript-learning/topic/${currentModule.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
...
export default TypeScriptTopicPage;
