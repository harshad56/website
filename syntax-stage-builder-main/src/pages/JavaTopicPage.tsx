...
const JavaTopicPage = () => {
  ...
  const moduleData = useMemo(
    () => javaModules.find((module) => module.id === moduleNumber),
    [moduleNumber]
  );

  const currentTopic = useMemo(
    () => moduleData?.topics.find((topic) => topic.id === topicId),
    [moduleData, topicId]
  );

  const [userCode, setUserCode] = useState(currentTopic?.codeExample ?? "");
  ...
  const currentModuleTopics = moduleData?.topics ?? [];
  const topicIndex = currentModuleTopics.findIndex((topic) => topic.id === currentTopic?.id);
  const previousTopic = topicIndex > 0 ? currentModuleTopics[topicIndex - 1] : null;
  const nextTopic =
    topicIndex >= 0 && topicIndex < currentModuleTopics.length - 1
      ? currentModuleTopics[topicIndex + 1]
      : null;

  const runExample = () => {
    setOutput("Code executed successfully!\n(Use your local environment to run Java code.)");
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">
            The topic you are looking for does not exist. Please return to the Java course.
          </p>
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button variant="outline" onClick={() => setUserCode(currentTopic.codeExample)}>
                    Load into editor
                  </Button>
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Use this sandbox to sketch solutions or pseudo-code.</CardDescription>
              </CardHeader>
              <CardContent>
                <textarea
                ...
                <div className="flex gap-3 mt-4">
                  <Button onClick={() => setOutput("Code compiled successfully!")}>▶ Run Code</Button>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mt-8">
          <Button
            ...
            onClick={() => {
              if (previousTopic) {
                navigate(`/java-learning/topic/${moduleData.id}/${previousTopic.id}`);
              }
            }}
          >
            ← Previous Topic
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (nextTopic) {
                  navigate(`/java-learning/topic/${moduleData.id}/${nextTopic.id}`);
                }
              }}
            >
              Next Topic →
